...
export default function Profile() {
    ...
    useEffect(() => {
        if (profile) {
            setProgrammingKnowledge(profile.programming_knowledge || 'beginner');
            setPriorRoboticsExperience(profile.prior_robotics_experience || false);
            setLearningGoals(profile.learning_goals || []);
            setLearningStyle(profile.preferred_learning_style || 'mixed');
        }
    }, [profile]);

    // Redirect if not logged in - BUT WAIT for loading to complete first!
    useEffect(() => {
        if (!loading && !user) {
